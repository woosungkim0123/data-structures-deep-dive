...
다익스트라 알고리즘은 최단 경로를 찾는 알고리즘으로, 구글 지도나 자동차 네비게이션과 같은 경로 탐색 시스템에 널리 사용됩니다. 이 알고리즘은 음수 가중치가 없는 그래프에서 효과적으로 작동하며, 실제 세계의 많은 상황에 적용할 수 있습니다.

### 다익스트라 알고리즘의 작동 원리 

하나의 출발 정점에서 다른 모든 정점까지의 최단 경로를 찾습니다.

다익스트라 알고리즘은 탐욕(Greedy) 알고리즘이면서 동시에 동적 계획법(DP)의 특성을 갖습니다. 이는 작은 하위 문제들의 해결책을 반복적으로 활용하여 전체 문제를 효율적으로 해결합니다.

> 동적 계획법은 복잡한 문제를 해결하기 위해 작은 문제들로 나누고, 이들 작은 문제들의 결과를 재활용하는 방식으로 중복된 계산을 최소화하는데 중점을 두는 알고리즘

### 다익스트라 알고리즘과 플로이드 알고리즘의 차이

다익스트라 알고리즘은 **한 출발점에서 다른 모든 점까지**의 최단 경로를 찾습니다.

반면, 플로이드 알고리즘은 **모든 정점에서 모든 다른 정점까지**의 최단 경로를 찾습니다.

### 진행 방식

- 초기화: 시작 정점을 설정하고, 최단 경로 비용을 나타내는 테이블을 초기화합니다.
- 경로 탐색: 시작 정점에서 갈 수 있는 인접 정점들 중 최소 비용의 경로를 선택합니다.
- 업데이트: 선택된 정점을 통해 다른 정점까지의 비용을 업데이트하고, 이 과정을 반복합니다.
- 최단 경로 결정: 모든 정점을 방문하며 최단 경로를 결정합니다.
